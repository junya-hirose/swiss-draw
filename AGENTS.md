...
- Cross-device realtime sync is not implemented.
- QR display is a deterministic placeholder, not a real QR code.
- Draw result input is incomplete.
- Detailed tiebreakers such as OMW%, GW%, and OGW% are not implemented.
...
- Replace placeholder QR rendering with a real QR library.
