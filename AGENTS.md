...
- Manual drops: `Player.dropped` can be toggled from the admin standings panel; dropped players keep their results and are excluded from future pairings.
- Penalties are kept for the whole event (per-match `judgeActions`), listed in the admin penalty log, and exported as a flat `penalties` array in the results JSON for external services.
- Participant identity is locked per browser per event (`swiss-draw-participant-lock:*`); once confirmed, the participant view cannot switch to another player.
- BO1 participant reporting is two buttons (win/lose) instead of score entry.
- Judge calls per table are recorded with call time, live elapsed display, and suggested time extensions.
